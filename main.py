...
async def save_filter_value(message: Message, state: FSMContext):
    ...
    if state_name == "city":
        ...
        try:
            ...
            await message.answer("❌ Формат: 5-10")
            return
        ...
        try:
            ...
            await message.answer("❌ Формат: 4.0")
            return

    save_filters(filters)
    await message.answer(f"✅ Збережено фільтр: {state_name.upper()}")
    await state.clear()
...
async def check():
    try:
        ...
        await bot.send_message(GROUP_ID, f"❌ Помилка: {e}")
        return {"error": str(e)}
